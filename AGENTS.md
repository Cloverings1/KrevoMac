...
**Auth flow:**
`ASWebAuthenticationSession` → `https://www.krevo.io/mac-auth` (Clerk) → server creates device token → redirects to `krevo://auth?token=xxx` → `AuthManager` consumes the callback only while sign-in is active → `AuthView` passes the token to `AppState.signIn(token:)` → Keychain
...
| `chunkTimeout` | 60s | Per-chunk stall timeout |
